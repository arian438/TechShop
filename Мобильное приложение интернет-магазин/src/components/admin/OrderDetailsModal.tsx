...
export function OrderDetailsModal({ 
  ...
  const getStatusBadge = (status: string) => {
    const statusMap = {
      ...
      'processing': { label: 'В обработке', variant: 'default' as const, icon: Package },
      'paid': { label: 'Оплачен', variant: 'default' as const, icon: CreditCard },
      'shipping': { label: 'В пути', variant: 'default' as const, icon: Truck },
      'delivered': { label: 'Доставлен', variant: 'outline' as const, icon: CheckCircle },
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                <div className="font-medium">{customer?.name || 'Неизвестный клиент'}</div>
